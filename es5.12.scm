...
(define (make-new-machine)
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
      ...
      ; (merge-sort (delete-duplicates (map car seq))
      ;             (lambda (x y)
      ;               (string<? (symbol->string x)
      ;                         (symbol->string y)))))
      ;define (extract-registers-for-entry-points ops)
      ; (delete-duplicates (map (lambda (op)
      ;                           (register-exp-reg (car (operation-exp-operands op))))
      ;                         (filter (lambda (op) 
      ;                                   (eq? 'goto (operation-exp-op op)))
      ;                                 ops))))
        ...
        (cond ((eq? message 'start)
              ...
              ((eq? message 'install-operations)
               (lambda (ops)
                 ...
                 ;(set! registers-for-entry-points (extract-registers-for-entry-points ops))
